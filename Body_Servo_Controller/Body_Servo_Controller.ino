...
#define CPU_ARM_RAISE             0X0040 //b0000000001000000
#define CPU_ARM_ROTATE             0x0080 //b0000000010000000
#define CPU_ARM_EXTEND             0x0100 //b0000000100000000
...
const ServoSettings servoSettings[] PROGMEM = {
    { 1,  2200,  200, TOP_UTILITY_ARM },      /* 0: Top Utility Arm */
    { 2,  2400,  400, BOTTOM_UTILITY_ARM },   /* 1: Bottom Utility Arm */
    { 3,   859, 1600, LARGE_LEFT_DOOR },      /* 2: Large Left door as viewing from looking at R2 */
    { 4,  2250, 1450, LARGE_RIGHT_DOOR },     /* 3: Large Right Door as viewing from looking at R2 */
    { 5,  2300, 1192, CHARGE_BAY_DOOR },      /* 4: Charge Bay Inidicator Door*/
    { 6,  2300, 1390, DATA_PANEL_DOOR },      /* 5: Data Panel Door */
    { 7,  1900, 600, CPU_ARM_RAISE },        /* 10: CPU Arm Raise/Lower */
    { 8,  700, 2300, CPU_ARM_ROTATE },       /* 11: CPU Arm Rotate */
    { 9,  1050, 2450, CPU_ARM_EXTEND },      /* 12: CPU Arm Extend */
  };
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
void turnOnCBI(){
  sendESPNOWCommand("BC",":LI99");
};
void turnOffCBI(){
  sendESPNOWCommand("BC",":LI98");
};

void turnOnDataPanel(){
    sendESPNOWCommand("BC",":LH99");
}
void turnOffDataPanel(){
    sendESPNOWCommand("BC",":LH98");
}

void turnOnCBIandDataPanel(){
    sendESPNOWCommand("BC",":LJ99");
}
void turnOffCBIandDataPanel(){
    sendESPNOWCommand("BC",":LJ98");
}
...
void shortCircuit(int servoBoard, int servoEasingMethod, uint32_t varSpeedMin, uint32_t varSpeedMax, uint32_t delayCallDuration) {
  ...
  fVarSpeedMax = varSpeedMax;                                                               // sets Global Variable from the local variable to allow the lambda function to utilize it
if (delayCallDuration == 0){delayCallDuration = defaultESPNOWSendDuration;}               //sets default delayCall to allow time for ESP-NOW message to get to reciever ESP.
  snprintf(stringToSend, sizeof(stringToSend),":D205E%02d%04d%04d", servoEasingMethod, varSpeedMin, varSpeedMax);
  setServoEasingMethod(servoEasingMethod);
  switch(servoBoard){
    case 1: SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelShortCircuit, ALL_SERVOS_MASK, fVarSpeedMin, fVarSpeedMax); break;
    case 2: sendESPNOWCommand("DC", stringToSend); break;
    case 3: SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelShortCircuit, ALL_SERVOS_MASK, varSpeedMin, varSpeedMax); 
            DelayCall::schedule([]{sendESPNOWCommand("DC", stringToSend);}, delayCallDuration); break;
    case 4: sendESPNOWCommand("DC", stringToSend); 
...
void WaveUtilityArm(int servoBoard, int servoEasingMethod, uint32_t varSpeedMin, uint32_t varSpeedMax, uint32_t delayCallDuration) {
  // Command: Dx05
  // add sequence for this routine.  
  fVarSpeedMin = varSpeedMin;                                                               // sets Global Variable from the local variable to allow the lambda function to utilize it
  fVarSpeedMax = varSpeedMax;                                                               // sets Global Variable from the local variable to allow the lambda function to utilize it

  SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelWaveUtilityArms, TOP_UTILITY_ARM, fVarSpeedMin, fVarSpeedMax);
  D_command[0] = '\0';
};
...
void allOpenClose(int servoBoard, int servoEasingMethod, uint32_t varSpeedMin, uint32_t varSpeedMax, uint32_t delayCallDuration){
  ...
  setServoEasingMethod(servoEasingMethod);
  switch(servoBoard){
    case 1: SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelAllOpenClose, ALL_SERVOS_MASK, fVarSpeedMin, fVarSpeedMax);  turnOnCBIandDataPanel(); break;
    case 2: sendESPNOWCommand("DC", stringToSend); break;
...
void allOpenCloseLong(int servoBoard, int servoEasingMethod, uint32_t varSpeedMin, uint32_t varSpeedMax, uint32_t delayCallDuration){
  ...
  switch(servoBoard){
    case 1: turnOnCBIandDataPanel();
            SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelAllOpenCloseLong, ALL_SERVOS_MASK, fVarSpeedMin, fVarSpeedMax); break;
    case 2: sendESPNOWCommand("DC", stringToSend); break;
    ...
    case 4: turnOnCBIandDataPanel();
      ...
      DelayCall::schedule([]{turnOffCBIandDataPanel();}, 10000);
...
void allFlutter(int servoBoard, int servoEasingMethod, uint32_t varSpeedMin, uint32_t varSpeedMax, uint32_t delayCallDuration){
  ...
  switch(servoBoard){
    case 1: turnOnCBIandDataPanel();
            SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelAllFlutter, ALL_SERVOS_MASK, fVarSpeedMin, fVarSpeedMax); break;
    case 2: sendESPNOWCommand("DC", stringToSend); break;
    case 3: turnOnCBIandDataPanel();
    ...
    case 4: turnOnCBIandDataPanel();
        ...
        DelayCall::schedule([]{turnOffCBIandDataPanel();}, 1400);
  D_command[0]   = '\0';   
};
...
void allOpenCloseRepeat(int servoBoard, int servoEasingMethod, uint32_t varSpeedMin, uint32_t varSpeedMax, uint32_t delayCallDuration){
  ...
  switch(servoBoard){
    case 1: turnOnCBIandDataPanel();
            SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelAllFOpenCloseRepeat, ALL_SERVOS_MASK, fVarSpeedMin, fVarSpeedMax); break;
    case 2: sendESPNOWCommand("DC", stringToSend); break;
    case 3: turnOnCBIandDataPanel();
...
void panelWave(int servoBoard, int servoEasingMethod, uint32_t varSpeedMin, uint32_t varSpeedMax, uint32_t delayCallDuration){
  ...
  switch(servoBoard){
    case 1: turnOnCBIandDataPanel();
            SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelWave, ALL_SERVOS_MASK, fVarSpeedMin, fVarSpeedMax); break;
    case 2: sendESPNOWCommand("DC", stringToSend); break;
    case 3: turnOnCBIandDataPanel();
            SEQUENCE_PLAY_ONCE_VARSPEED(servoSequencer, SeqPanelWave, ALL_SERVOS_MASK, fVarSpeedMin, fVarSpeedMax);
            DelayCall::schedule([] {sendESPNOWCommand("DC", stringToSend);}, delayCallDuration); break;
    case 4: turnOnCBIandDataPanel();
            sendESPNOWCommand("DC", stringToSend);
